...
function timeStr2hour(s) {
    ...
    s = s.split(':');
    if (s.length == 1) a = s[0].substr(0, 2) - 0, b = s[0].substr(2, 2) - 0, c = s[0].substr(4, 2) - 0; else if (s.length == 2) a = s[0] - 0, b = s[1] - 0, c = 0; else a = s[0] - 0, b = s[1] - 0, c = s[2] - 0;
...
Number.prototype.toFixed = function (m) {
    var n = this, f = '', p = Math.pow(10, m);
    if (n < 0) n = -n, f = '-';
    var a = Math.floor(n), b = n - a;
    b = Math.round(b * p);
    if (b >= p) a++, b -= p;
    if (m) b = '.' + (p + b + '').substr(1, m); else b = '';
    return f + a + b;
};
...
var JD = {
    ...
    }, Y: 2000, M: 1, D: 1, h: 12, m: 0, s: 0, toJD: function () {
    ...
    }, Weeks: new Array('日', '一', '二', '三', '四', '五', '六', '七'), getWeek: function (jd) {
...
function XL0_calc(xt, zn, t, n) {
    ...
    if (xt == 0) {
        ...
        if (zn == 0) v += -3 * t / rad;
        if (zn == 2) v += dv / 1000000; else v += dv / rad;
    }
    return v;
}
...
function sunShengJ(jd, L, fa, sj) {
    ...
    for (i = 0; i < 2; i++) {
        var T = jd / 36525, E = (84381.4060 - 46.836769 * T) / rad;
...
function dituJM(p, Jb, Wb) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (c >= 'a' && c <= 'z') b = c.charCodeAt(0) - 97; else if (c >= 'A' && c <= 'Z') b = 65 - c.charCodeAt(0); else {
        ...
        k2++;
        if (k2 % 2) J += b, a[k] = J * Jb, k++; else W += b, a[k] = W * Wb, k++;
    }
    return a;
}
...
var touY = {
    MollY: new Array(), MollCZ: function (W) {
...
var SZJ = {
    L: 0, fa: 0, dt: 0, E: 0.409092614, getH: function (h, w) {
        ...
        if (H0) r.H0 = this.getH(0.7275 * cs_rEar / z[2] - 34 * 60 / rad, z[1]);
    }, Mt: function (jd) {
        this.dt = dt_T(jd);
        ...
        z = llrConv(z, this.E);
        r.H = rad2rrad(pGST(jd, this.dt) + this.L - z[0]);
        if (xm == 10 || xm == 1) r.H1 = this.getH(-50 * 60 / rad, z[1]);
        ...
        r.x += rad2rrad(Math.PI - r.H) / sv;
        return r;
    }, rts: new Array(), calcRTS: function (jd, n, Jdl, Wdl, sq) {
        var i, c, r;
        if (!this.rts.length) {
...
function daJu(xt, t, dx) {
    ...
    if (dx) b = c[3] / 36525; else b = c[4] / 36525;
    t = b + a * int2((t - b) / a + 0.5);
    var i, dt, r1, r2, r3;
    for (i = 0; i < 3; i++) {
        dt = c[i] / 36525;
...
function xingLiu(xt, t, sn) {
    ...
    var v = pi2 / hh;
    if (xt > 2) v = -v;
    for (i = 0; i < 6; i++) t -= rad2rrad(XL0_calc(xt, 0, t, 8) - XL0_calc(0, 0, t, 8)) / v;
...
function xingHR(xt, t, f) {
    ...
    if (xt > 2) v = -v;
    for (i = 0; i < 6; i++) t -= rad2rrad(XL0_calc(xt, 0, t, 8) - XL0_calc(0, 0, t, 8) - w0) / v;
...
function ecFast(jd) {
    ...
    re.ac = 1, re.lx = 'N';
    if (Math.abs(Math.sin(L)) > 0.4) return re;
    ...
    var ls1, ls2, ls3, ls4;
    if (fh2 < h) re.lx = 'T'; else re.lx = 'A';
    ls1 = Agm - (b + r1);
    if (Math.abs(ls1) < 0.016) re.ac = 0;
    ls2 = Agm - (b + Ar2);
    if (Math.abs(ls2) < 0.016) re.ac = 0;
    ls3 = Agm - (b);
    if (Math.abs(ls3) < 0.016) re.ac = 0;
    ls4 = Agm - (b - Ar2);
    if (Math.abs(ls4) < 0.016) re.ac = 0;
    if (ls1 > 0) re.lx = 'N'; else if (ls2 > 0) re.lx = 'P'; else if (ls3 > 0) re.lx += '0'; else if (ls4 > 0) re.lx += '1'; else {
...
var rsGS = {
    ...
    init: function (jd, n) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (n == 7) S = e_coord(T, -1, -1, -1), M = m_coord(T, -1, -1, -1);
            if (n == 3) S = e_coord(T, 65, 65, 65), M = m_coord(T, -1, 150, 150);
            ...
            M = llrConv(M, E);
            S[2] *= cs_AU;
            if (i && S[0] < a[0]) S[0] += pi2;
            if (i && M[0] < a[3]) M[0] += pi2;
            k = i * 9;
    ...
    chazhi: function (jd, xt) {
        ...
        if (c > N - 2) c = N - 2;
        t -= c, p += c * w;
        for (i = 0; i < m; i++, p++) z[i] = B[p] + (B[p + w] - B[p - w] + (B[p + w] + B[p - w] - B[p] * 2) * t) * t / 2;
        return z;
    },
    sun: function (jd) {
    ...
    mQie: function (M, vx0, vy0, h, r, I, A) {
        ...
        if (A.f2 != A.f) {
            ...
            if (g.n) {
                if (A.f) dj = g.R2, F = g.B; else dj = g.R1, F = g.A;
                F[2] = 0;
    ...
    mDian: function (M, vx0, vy0, AB, r, I, A) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (!p.n) break;
            if (AB) a = p.A, R = p.R1; else a = p.B, R = p.R2;
        }
        if (p.n && R <= r) {
    ...
    jieX: function (jd) {
        ...
        for (i = 0; i <= N; i++, t += dt) {
            ...
            if (p[1] != 100 && n4 == 0 || p[1] == 100 && n4 == 1) {
                ...
                var dj;
                if (n4 == 0) dj = ls.R2, ls = ls.B; else dj = ls.R1, ls = ls.A;
                ls[2] = 0;
...
var rsPL = {
    ...
    secXY: function (jd, L, fa, high, re) {
        ...
        if (this.nasa_r) re.mr *= cs_sMoon2 / cs_sMoon;
    ...
    secMax: function (jd, L, fa, high) {
        ...
        if (rmin <= G.mr + G.sr) {
            ...
            this.b1 = G.mr / G.sr;
            this.sun_s = sunShengJ(jd - dt_T(jd) + L / pi2, L, fa, -1);
            this.sun_j = sunShengJ(jd - dt_T(jd) + L / pi2, L, fa, 1);
...
function JWdecode(v) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (v[i] > 96) v[i] -= 97 - 36; else if (v[i] > 64) v[i] -= 65 - 10; else v[i] -= 48;
    }
...
for (i = 0; i < SQv.length; i++) SQv[i] = SQv[i].split(",");
var oba = {
...
var obb = {
    ...
    ymc: new Array('十一', '十二', '正', '二', '三', '四', '五', '六', '七', '八', '九', '十'),
    rmc: new Array('初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十', '十一', '十二', '十三', '十四', '十五', '十六', '十七', '十八', '十九', '二十', '廿一', '廿二', '廿三', '廿四', '廿五', '廿六', '廿七', '廿八', '廿九', '三十', '卅一'),
,李晔,龙纪,890,2,0,唐,昭宗,李晔,大顺,892,2,0,唐,昭宗,李晔,景福,894,5,0,唐,昭宗,李晔,乾宁,898,4,0,唐,昭宗,李晔,光化,' + '901,4,0,唐,昭宗,李晔,天复,904,1,0,唐,昭宗,李晔,天佑,905,3,1,唐,昭宣帝,李祝,天佑,907,5,0,五代/梁,太祖,朱温,开平,911,2,0,五代/梁,太祖,朱温,乾化,913,1,0,五代/梁,庶人,朱友圭,凤历,913,3,2,五代/梁,末帝,朱友贞,乾化,915,7,0,五代/梁,末帝,朱友贞,贞明,921,3,0,五代/梁,末帝,朱友贞,龙德,923,4,0,五代/唐,庄宗,李存??,同光,926,5,0,五代/唐,明宗,李嗣源,天成,930,4,0,五代/唐,明宗,李嗣源,长兴,934,1,0,五代/唐,闵帝,李从厚,应顺,934,3,0,五代/唐,潞王,李从珂,清泰,936,6,0,五代/晋,高祖,石敬瑭,天福,' + '942,2,6,五代/晋,出帝,石重贵,天福,944,3,0,五代/晋,出帝,石重贵,开运,947,12,0,五代/汉,高祖,刘知远,天福,948,1,0,五代/汉,隐帝,刘承?v,乾?v,948,3,0,五代/汉,隐帝,刘承?v,乾?v,951,3,0,五代/周,太祖,郭威,广顺,954,1,0,五代/周,太祖,郭威,显德,954,6,0,五代/周,世宗,柴荣,显德,959,2,5,五代/周,恭帝,郭宗训,显德,960,4,0,北宋,太祖,赵匡胤,建隆,963,6,0,北宋,太祖,赵匡胤,乾德,968,9,0,北宋,太祖,赵匡胤,开宝,976,9,0,北宋,太宗,赵炅,太平兴国,984,4,0,北宋,太宗,赵炅,雍熙,988,2,0,北宋,太宗,赵炅,端拱,' + '990,5,0,北宋,太宗,赵炅,淳化,995,3,0,北宋,太宗,赵炅,至道,998,6,0,北宋,真宗,赵恒,咸平,1004,4,0,北宋,真宗,赵恒,景德,1008,9,0,北宋,真宗,赵恒,大中祥符,1017,5,0,北宋,真宗,赵恒,天禧,1022,1,0,北宋,真宗,赵恒,乾兴,1023,10,0,北宋,仁宗,赵祯,天圣,1032,2,0,北宋,仁宗,赵祯,明道,1034,5,0,北宋,仁宗,赵祯,景?v,1038,3,0,北宋,仁宗,赵祯,宝元,1040,2,0,北宋,仁宗,赵祯,康定,1041,8,0,北宋,仁宗,赵祯,庆历,1049,6,0,北宋,仁宗,赵祯,皇?v,1054,3,0,北宋,仁宗,赵祯,至和,' + '1056,8,0,北宋,仁宗,赵祯,嘉?v,1064,4,0,北宋,英宗,赵曙,治平,1068,10,0,北宋,神宗,赵顼,熙宁,1078,8,0,北宋,神宗,赵顼,元丰,1086,9,0,北宋,哲宗,赵煦,元?v,1094,5,0,北宋,哲宗,赵煦,绍圣,1098,3,0,北宋,哲宗,赵煦,元符,1101,1,0,北宋,徽宗,赵佶,建中靖国,1102,5,0,北宋,徽宗,赵佶,崇宁,1107,4,0,北宋,徽宗,赵佶,大观,1111,8,0,北宋,徽宗,赵佶,政和,1118,2,0,北宋,徽宗,赵佶,重和,1119,7,0,北宋,徽宗,赵佶,宣和,1126,2,0,北宋,钦宗,赵桓,靖康,1127,4,0,南宋,高宗,赵构,建炎,' + '1131,32,0,南宋,高宗,赵构,绍兴,1163,2,0,南宋,孝宗,赵慎,隆兴,1165,9,0,南宋,孝宗,赵慎,乾道,1174,16,0,南宋,孝宗,赵慎,淳熙,1190,5,0,南宋,光宗,赵暴,绍熙,1195,6,0,南宋,宁宗,赵扩,庆元,1201,4,0,南宋,宁宗,赵扩,嘉泰,1205,3,0,南宋,宁宗,赵扩,开禧,1208,17,0,南宋,宁宗,赵扩,嘉定,1225,3,0,南宋,理宗,赵昀,宝庆,1228,6,0,南宋,理宗,赵昀,绍定,1234,3,0,南宋,理宗,赵昀,端平,1237,4,0,南宋,理宗,赵昀,嘉熙,1241,12,0,南宋,理宗,赵昀,淳?v,1253,6,0,南宋,理宗,赵昀,???v,' + '1259,1,0,南宋,理宗,赵昀,开庆,1260,5,0,南宋,理宗,赵昀,景定,1265,10,0,南宋,度宗,赵?Q,咸淳,1275,2,0,南宋,恭宗,赵(上“日”下“丝”),德?v ,1276,3,0,南宋,端宗,赵(上“日”下“正”),景炎,1278,2,0,南宋,帝?m,赵?m,祥兴,1271,24,7,元,世祖,孛儿只斤·忽必烈,至元,1295,3,0,元,成宗,孛儿只斤·铁穆耳,元贞,1297,11,0,元,成宗,孛儿只斤·铁穆耳,大德,1308,4,0,元,武宗,孛儿只斤·海山,至大,1312,2,0,元,仁宗,孛儿只斤·爱育黎拔力八达,皇庆,1314,7,0,元,仁宗,孛儿只斤·?塾?黎拔力八?_,延?v,1321,3,0,元,英宗,孛儿只斤·宗硕德八剌,至治,1324,5,0,元,泰定帝,孛儿只斤·也孙铁木耳,泰定,1328,1,0,元,泰定帝,孛儿只斤·也孙铁木耳,至和,' + '1328,1,0,元,幼主,孛儿只斤·阿速吉八,天顺,1328,3,0,元,文宗,孛儿只斤·图贴睦尔,天历,1330,3,0,元,文宗,孛儿只斤·图贴睦尔,至顺,1333,3,0,元,惠宗,孛儿只斤·妥镤贴睦尔,元统,1335,6,0,元,惠宗,孛儿只斤·妥镤贴睦尔,至元,1341,28,0,元,惠宗,孛儿只斤·妥镤贴睦尔,至正,1368,31,0,明,太祖,朱元璋,洪武,1399,4,0,明,惠帝,朱允??,建文,1403,22,0,明,成祖,朱棣,永乐,1425,1,0,明,仁宗,朱高炽,洪熙,1426,10,0,明,宣宗,朱瞻基,宣德,1436,14,0,明,英宗,朱祁镇,正统,1450,7,0,明,代宗,朱祁钰,景泰,1457,8,0,明,英宗,朱祁镇,天顺,1465,23,0,明,宪宗,朱见深,成化,' + '1488,18,0,明,孝宗,朱?v樘,弘治,1506,16,0,明,武宗,朱厚照,正德,1522,45,0,明,世宗,朱厚??,嘉靖,1567,6,0,明,穆宗,朱载贺,隆庆,1573,48,0,明,神宗,朱翊钧,万历,1620,1,0,明,光宗,朱常洛,泰昌,1621,7,0,明,熹宗,朱同校,天启,1628,17,0,明,毅宗,朱由检,崇祯,1644,18,0,清,世祖,爱新觉罗福临,顺治,1662,61,0,清,圣祖,爱新觉罗玄烨,康熙,1723,13,0,清,世宗,爱新觉罗胤?G,雍正,1736,60,0,清,高宗,爱新觉罗弘历,乾隆,1796,25,0,清,仁宗,爱新觉罗?J琰,嘉庆,1821,30,0,清,宣宗,爱新觉罗?F宁,道光,1851,11,0,清,文宗,爱新觉罗奕?},咸丰,' + '1862,13,0,清,穆宗,爱新觉罗载淳,同治,1875,34,0,清,德宗,爱新觉罗载??,光绪,1909,3,0,清,无朝,爱新觉罗溥仪,宣统,1912,37,0,近、现代,中华民国,,民国,1949,9999,1948,当代,中国,,公历纪元';
        this.JNB = s.split(',');
    ...
    getDayName: function (u, r) {
        ...
        if (u.Ljq) {
            if (u.Ljq == "清明") r.A += u.Ljq + ' ', r.Fjia = 1; else r.B += u.Ljq + ' ';
        }
        var w, w2;
        if (u.cur_dz >= 0 && u.cur_dz < 81) {
            w = obb.numCn[Math.floor(u.cur_dz / 9) + 1];
            if (u.cur_dz % 9 == 0) r.B += '『' + w + '九』 '; else r.C += w + '九第' + (u.cur_dz % 9 + 1) + '天 ';
        }
...
function Lunar() {
    ...
    this.yueLiHTML = function (By, Bm, curJD) {
        ...
        if (c.length > 33) c = '<span style="font-size:12px">' + c + '</span>'; else c = '<span style="font-size:16px;font-weight:bold">' + c + '</span>';
        var ta0 = '<tr><td colspan=7 style="background-color:#0000A0" style="color=#FFFF00">' + c + '</td></tr>';
        ...
        for (i = 0; i < this.dn; i++) {
            ...
            if (ob.Fjia) c2 = sty_date2; else c2 = sty_date;
...
var ht_b = {
    Vel: function (s) {
        ...
        var s;
        if (name == "xx" || "arr") s = 'stroke';
        if (name == "txt") s = 'textBox  inset="5px,5px,5px,5px" style="font-size:12px"';
        if (name == "sh") s = 'shadow on="T" type="single" color="#b3b3b3" offset="5px,5px"';
        if (name == "ext") s = 'extrusion on="True" color="red" rotationangle="0,0"';
        if (name == "fill") s = "fill";
        var el2 = ht_b.Vel(s);
        ...
        el.setqb = function (qb) {
            if (!el.qbID) this.qbID = ht_b.Ael(el, "xx");
        ...
        var obj = new Object();
        for (var ns in ob) {
            if (typeof(ob[ns]) == "object") obj[ns] = ht.clone(ob[ns]); else obj[ns] = ob[ns];
        }
        return obj;
    }
};
var ht = {
    ...
    shape: function (path) {
        ...
        if (ph.substr(0, 1) == ",") ph = ph.substr(1, ph.length - 1);
        el.phs = ph.split(',');
    ...
    quxian: function (points, qz) {
        ...
        s = s.replace(/(^,)*/g, '');
        if (qz == 'q') s = 'm ' + s + ' qb ' + points + 'r 0,0 e';
        if (qz == 'z') s = 'm ' + s + ' l ' + points + ' e';
        if (qz == 'c') s = 'm ' + s + ' c ' + points + ' e';
        return this.shape(s);
    },
    cls: function () {
...
var tu1 = {
    ...
    move2a: function (J1, W1, J2, W2, mr, sr) {
        ...
        dJ *= Math.cos((W1 + W2) / 2) * rad, dW *= rad;
        dJ = v / 32 * dJ / 60, dW = v / 32 * dW / 60;
        sr = v / 32 * 2 * sr / 60, mr = v / 32 * 2 * mr / 60;
    ...
    move2b: function (J1, W1, J2, W2, mr, er, Er) {
        ...
        dJ *= Math.cos((W1 + W2) / 2) * rad, dW *= rad;
        dJ = v / 32 * dJ / 60, dW = v / 32 * dW / 60;
        er = v / 32 * 2 * er / 60, Er = v / 32 * 2 * Er / 60, mr = v / 32 * 2 * mr / 60;
        if (Math.abs(dJ) > 3.5 * this.dx || Math.abs(dW) > 2.5 * this.dy) {
            this.ecShow(0, 0, 0, 0);
            return;
        }
...
var tu2 = {
    isInit: 0, eR: 150, x0: 240, y0: 280, init: function (can) {
        ht.canvas = can;
        if (this.isInit) return;
        this.isInit = 1;
        this.w = String(can.style.width).replace(/[^0-9]/g, '');
        ...
        if (!hd) {
            ...
            if (a.I[1] < Math.PI / 2) this.dbj.fillcolor = '#00a0FF'; else this.dbj.fillcolor = '#000000';
...
var tu3 = {
    isInit: 0, eR: 250, x0: 350, y0: 300, init: function (can) {
        ht.canvas = can;
        if (this.isInit) return;
        this.isInit = 1;
        this.w = String(can.style.width).replace(/[^0-9]/g, '');
        this.h = String(can.style.height).replace(/[^0-9]/g, '');
        can.coordsize = this.w + ',' + this.h;
        this.p = ht.shape(''), this.p.filled = false, this.p.strokeColor = '#E0E0E0';
        this.P1 = ht.shape(''), this.P1.filled = false, this.P1.strokeColor = '#808080';
        this.P2 = ht.shape(''), this.P2.filled = false, this.P2.strokeColor = '#D0D0FF';
        this.A = ht.shape(''), this.A.filled = false, this.A.strokeColor = '#FF6060';
        this.B = ht.shape(''), this.B.filled = false, this.B.strokeColor = '#80F080';
        this.C = ht.shape(''), this.C.filled = false, this.C.strokeColor = '#8080F0';
        this.D = ht.shape(''), this.D.filled = false, this.D.strokeColor = '#000000';
        this.eth = ht.oval(0, 0, this.eR * 2, this.eR * 2);
        this.eth.filled = false;
        this.eth.moveto2(this.x0, this.y0);
        ...
        for (var i = 0; i < d.length; i += 2) {
            ...
            if (c) p.p_moveto(x, y); else p.p_lineto(x, y);
        }
    }, showDitu: function () {
        this.P1.p_cls();
        this.P2.p_cls();
...
function GJ1_calc1() {
    ...
    if (jd < 0) GJ1_out.innerHTML = '不能为负'; else GJ1_out.innerHTML = JD.JD2str(jd);
}
...
function GJ1_calc2(fs) {
    ...
    if (fs == 2) jd -= JD.JD(year2Ayear(GJ1_y.value), 1, 1.5) - 1;
...
function GJ2_pi2() {
    ...
    for (i = 0; i < 25; i++) {
        a *= 2;
        T /= 2 + sqrt(4 * R * R - T) / R, T = GJ2_jie(T, R);
        if (T < R) break;
...
function GJ2_pi() {
    var N = GJ2_N.value - 0 + 1, a = new Array();
    var i = Math.round(3.4 * N), b = 2 * i + 1, j, f;
    for (j = 0; j < N; j++) a[j] = 0;
    for (; i > 0; i--, b -= 2, a[0] += 2) for (j = 0, f = 0; j < N; j++) {
        f = a[j] * i + f * 10;
        a[j] = Math.floor(f / b);
        f %= b;
    }
    for (i = N - 1; i > 0; i--) a[i - 1] += Math.floor(a[i] / 10), a[i] %= 10;
    a[0] = '<br>';
    for (i = 1; i < N; i++) {
        if (i % 10 == 0) a[i] += ' ';
        if (i % 100 == 0) a[i] += '<br>';
...
GJ2_machin = {
    add: function (a, b, n) {
        ...
        a[0] = v;
        a.length = n;
    }, a: new Array(), b: new Array(), c: new Array(), arctg: function (k, v, zf, N) {
        ...
        this.div(b, k, N);
        if (zf > 0) this.add(a, b, N); else this.sub0(a, b, a, N);
    }, pi: function () {
        var N = GJ2_N.value - 0;
        N = floor(N / 10 + 1.5);
